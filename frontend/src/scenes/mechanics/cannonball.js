function mapPoint(x, y, view, t) {
  ...
  return {
    x:
      left +
      ((x - view.xMin) / Math.max(0.001, view.xMax - view.xMin)) *
        (right - left),
    y:
      ground -
      ((y - view.yMin) / Math.max(0.001, view.yMax - view.yMin)) *
        (ground - top),
  };
}
function sky(ctx, t, frame) {
  const ground = t.height - 46,
    mode = PhysicsExperience.context(ctx, frame, "projectileField");
  if (!mode.environment) {
...
function target(ctx, t, frame) {
  const p = mapPoint(t.config.target, 0, t.config.view, t),
    surface = PhysicsVisuals.token(frame, "colors", "surface", "#FFF"),
    accent = PhysicsVisuals.token(frame, "colors", "net_force", "#C2410C");
    ...
    for (const track of Object.values(frame.tracks)) {
      ...
      PhysicsAnimation.fadingTrail(
        ...
        (q) => mapPoint(q.x, q.y || 0, frame.config.view, t),
        {
          color:
            track.style.color ||
            PhysicsVisuals.token(frame, "colors", "trajectory", "#1769AA"),
          width: 2.5,
          opacity: 0.42,
        },
      );
      ...
      PhysicsAssets.projectile(ctx, frame, {
        ...
        shadow: true,
        label:
          frame.config.tracks.length > 1 &&
          PhysicsVisuals.responsive(frame) !== "mobile"
            ? track.label
            : "",
      });
    }
