...
const scene = {
  draw(ctx, f) {
    const t = f.transform,
      W = t.width,
      H = t.height,
      c = f.config.earthTunnel || {},
      cx = W / 2,
      cy = H / 2,
      R = Math.min(W, H) * 0.34,
      X = (km) => cx + (km / f.config.radiusKm) * R;
    ...
    for (const [index, track] of Object.values(f.tracks).entries()) {
      const meta = c.travelers[index] || {},
        x = X(track.x),
        acceleration = track.y || 0,
        turn = X(meta.turningPointKm || 0),
        trail = f.trails.get(track.id) || [],
        dashed = index % 2 === 1;
      PhysicsAnnotations.pathGuide(ctx, f, {
        points: [
          { x: turn, y: cy - 23 },
      ...
      if (Math.abs(acceleration) > 1e-8)
        PhysicsAnnotations.vector(
          ...
          f,
          {
            ...
            fixed_length_px: 46,
            scale_disclosure:
              "Acceleration direction is physical; length is normalized",
          },
          f.progress,
          index === 0,
        );
