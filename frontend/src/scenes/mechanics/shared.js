...
const scene = {
  draw(ctx, s) {
    ...
    if (kind === "ramp") {
      const theta = ((c.angleDeg || 0) * Math.PI) / 180,
        maxRun = w - 140,
        run = Math.min(maxRun, (h - 150) / Math.max(Math.tan(theta), 0.001)),
        x0 = (w - run) / 2,
        y0 = 70,
        x1 = x0 + run,
        y1 = y0 + Math.tan(theta) * run;
      const rampHeight = Math.max(4, y1 - y0),
        progress = s.tracks.block?.x || 0,
        bx = x0 + (x1 - x0) * progress,
        by = y0 + (y1 - y0) * progress - 23;
      ...
      const cx = w / 2,
        cy = h * 0.55,
        turn = ((s.tracks.beam?.x || 0) * Math.PI) / 180,
        half = Math.min(w * 0.37, 260),
        maxArm = Math.max(c.loadArmM || 1, c.effortArmM || 1),
        scale = half / maxArm;
      ctx.save();
      ctx.translate(cx, cy);
      ctx.rotate(turn);
      ...
      const lx = -(c.loadArmM || 0) * scale,
        ex = (c.effortArmM || 0) * scale;
      PhysicsAssets.block(ctx, s, {
        x: lx,
        y: -29,
        width: 44,
      ...
      const car = s.tracks.car,
        pad = 62,
        top = 48,
        bottom = h - 58,
        length = c.trackLengthM || 1,
        maxHeight = Math.max(1, ...(c.trackPoints || []).map((q) => q.height)),
        X = (d) => pad + (d / length) * (w - pad * 2),
        Y = (v) => bottom - (v / maxHeight) * (bottom - top),
        points = (c.trackPoints || []).map((q) => ({
          x: X(q.distance),
          y: Y(q.height),
        })),
        reachable = Math.max(
      ...
      if (reachable < length) {
        const hStop = heightAt(reachable),
          stop = { x: X(reachable), y: Y(hStop) };
      ...
      const distance = (car?.x || 0) * length,
        height = heightAt(distance),
        speed = car?.y || 0,
        x = X(distance),
        y = Y(height) - 18,
        next = Math.min(length, distance + length * 0.01),
        slope = Math.atan2(
          Y(heightAt(next)) - Y(height),
          X(next) - X(distance),
        ),
        pe = (c.massKg || 0) * (c.gravityMps2 || 9.81) * height,
        ke = 0.5 * (c.massKg || 0) * speed * speed,
        lost = (c.lossPerMeterJ || 0) * distance;
      ...
      const cx = w / 2,
        cy = h * 0.52,
        total = s.tracks.body?.x || 0,
        angle = ((total % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2),
        radius = Math.min(82, w * 0.16);
      PhysicsAnnotations.pathGuide(ctx, s, {
        points: [
          { x: cx - radius - 18, y: cy },
          { x: cx + radius + 18, y: cy },
      ...
      if (c.shape === "Rod about center")
        ...
        PhysicsAssets.wheel(ctx, s, {
          ...
          fill:
            c.shape === "Hoop"
              ? PhysicsVisuals.token(s, "colors", "surface_muted", "#EAF0F6")
              : PhysicsVisuals.token(s, "colors", "uncertainty", "#6B7280"),
        });
