function drawBackground(ctx, t, frame) {
  const W = t.width,
    H = t.height,
    ground = H - 60,
    mode = PhysicsExperience.context(ctx, frame, "collisionTrack");
  if (!mode.environment) {
...
const scene = {
  onEvent(event, player) {
    if (event.type === "particle_burst") {
      ...
      const b = sample(player.config.tracks[1].x, event.fraction),
        t = player.coordinates();
  ...
  draw(ctx, frame) {
    ...
    const a = frame.tracks.car_a,
      b = frame.tracks.car_b,
      ax = t.x(a.x),
      bx = t.x(b.x);
    const trailA = frame.trails
      .get("car_a")
      .map((point) => ({ px: t.x(point.x) }));
    const trailB = frame.trails
      .get("car_b")
      .map((point) => ({ px: t.x(point.x) }));
    const impact = frame.config.impactFraction,
      since =
        (Math.max(0, frame.fraction - impact) * frame.config.durationMs) / 1000;
    if (
      frame.config.sticky &&
      frame.fraction >= impact &&
      frame.state !== "idle"
    ) {
