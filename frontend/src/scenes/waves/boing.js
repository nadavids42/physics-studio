...
const scene = {
  draw(ctx, s) {
    ...
    const wall = 48,
      span = Math.max(1, t.x(1) - t.x(0));
    tracks.forEach((track, index) => {
      const meta = c.oscillators[index] || {},
        cy = ((index + 1) * t.height) / (count + 1),
        equilibrium = t.x(0),
        x = t.x(track.x),
        velocity = track.y || 0,
        displacement = track.x || 0;
      ctx.save();
      ...
      const amplitude = Math.abs(meta.initialDisplacementM || 0),
        left = t.x(-amplitude),
        right = t.x(amplitude);
      PhysicsAnnotations.pathGuide(ctx, s, {
        points: [
      ...
      if ((meta.dampingNsM || 0) > 0 && !meta.driven) {
        const envelope =
            amplitude *
            Math.exp(
              (-(meta.dampingNsM || 0) * (meta.durationS || 0) * s.fraction) /
                (2 * (meta.massKg || 1)),
            ),
          elo = t.x(-envelope),
          ehi = t.x(envelope);
        PhysicsAnnotations.dimensionLine(ctx, s, {
          ...
          label: `damping envelope ±${envelope.toFixed(2)} m`,
        });
      }
