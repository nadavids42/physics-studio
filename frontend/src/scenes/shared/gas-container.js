...
const scene = {
  draw(ctx, s) {
    const w = s.transform.width,
      ...
      c = s.config.gasContainer || {},
      progress = s.reducedMotion ? 1 : s.fraction,
      left = w * 0.25,
      right = w * 0.75,
      outerTop = h * 0.16,
      bottom = h * 0.82,
      outerHeight = bottom - outerTop,
      maxV = Math.max(c.volumeA, c.volumeB, 0.001),
      v = c.volumeA + (c.volumeB - c.volumeA) * progress,
      gasH = 80 + (v / maxV) * outerHeight * 0.72,
      top = bottom - gasH,
      pressure = c.pressureA + (c.pressureB - c.pressureA) * progress,
      temp = c.tempA + (c.tempB - c.tempA) * progress;
    ...
    const limit =
      PhysicsExperience.level(s) === "diagram"
        ? Math.min(12, c.particleLayout.length)
        : c.particleLayout.length;
    for (let i = 0; i < limit; i++) {
      const particle = c.particleLayout[i],
        x = left + 12 + particle.u * (right - left - 24),
        y = top + 12 + particle.v * Math.max(8, bottom - top - 24);
    ...
    ctx.globalAlpha = 1;
    const arrowRatio = pressure / Math.max(c.pressureA, c.pressureB, 1),
      arrowLength = 24 + 28 * arrowRatio;
    for (const x of [
