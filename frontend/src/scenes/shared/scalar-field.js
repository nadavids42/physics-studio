...
const scene = {
  draw(ctx, s) {
    const w = s.transform.width,
      ...
      c = s.config,
      frames = c.fieldFrames || [],
      xs = c.fieldX || [],
      pad = { l: 58, r: 28, t: 46, b: 52 },
      left = pad.l,
      right = w - pad.r,
      top = pad.t,
      bottom = h - pad.b,
      mid = (top + bottom) / 2,
      limit = Math.max(0.001, c.fieldLimit || 1),
      X = (x) =>
        left +
        ((x - (xs[0] || 0)) /
          Math.max(0.001, (xs.at(-1) || 1) - (xs[0] || 0))) *
          (right - left),
      Y = (y) => mid - ((y / limit) * (bottom - top)) / 2;
    ...
    const fi = Math.min(
      ...
      roles = ["accent", "energy", "displacement", "electric_field"],
      styles = [[], [8, 5], [2, 4], [11, 4, 2, 4]];
    for (let source = 0; source < (c.sourceFrames || []).length; source++) {
      const ys = c.sourceFrames[source][fi],
      ...
      ctx.beginPath();
      ys.forEach((y, i) =>
        i ? ctx.lineTo(X(xs[i]), Y(y)) : ctx.moveTo(X(xs[i]), Y(y)),
      );
      ctx.stroke();
      ctx.restore();
    }
    ...
    sources.forEach((source, i) => {
      ...
      ctx.fillText(
        `Source ${i + 1} (${styles[i % styles.length].length ? "dashed" : "solid"}): φ=${source.phaseDeg.toFixed(0)}°`,
        left + (i % 2) * 210,
    ...
    ctx.fillText(
      `Resultant (heavy) · t=${c.fieldTimes[fi].toFixed(2)} s`,
      right - 230,
      bottom + 42,
    );
    ctx.restore();
