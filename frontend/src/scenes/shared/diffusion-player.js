  ...
  draw(ctx, s) {
    ...
    for (let j = 0; j < paths.length; j++) {
      const path = paths[j],
        ...
        start = Math.max(0, last - c.visibleWindowSegments),
        role = roles[j % roles.length],
        color = PhysicsVisuals.token(s, "colors", role, "#1769AA");
      ctx.save();
      ...
      for (let i = start + 1; i <= last; i++) {
        const a = path[i - 1],
          b = path[i],
          progress = (i - start) / Math.max(1, last - start);
      ...
      const origin = path[0],
        current = path[last],
        ox = cx + origin[0] * scale,
        oy = cy - (c.dimensions === 2 ? origin[1] : 0) * scale,
        x = cx + current[0] * scale,
        y = cy - (c.dimensions === 2 ? current[1] : 0) * scale;
      ctx.save();
      ctx.strokeStyle = color;
