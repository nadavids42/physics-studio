...
export const PhysicsAnimation = (() => {
  ...
  class Camera {
    ...
    focus(target = {}, durationMs = 320) {
      this.start = { ...this.current };
      this.target = {
        x: target.x ?? this.current.x,
        y: target.y ?? this.current.y,
        zoom: clamp(target.zoom ?? this.current.zoom, 0.25, 8),
      };
      this.elapsed = 0;
      this.duration = this.reducedMotion ? 0 : Math.max(0, durationMs) / 1000;
    ...
    update(seconds) {
      ...
      for (const key of ["x", "y", "zoom"])
        this.current[key] =
          this.start[key] + (this.target[key] - this.start[key]) * t;
    }
  ...
  function subtleMotionBlur(ctx, positions, draw, options = {}) {
    const reduced = options.reducedMotion || false,
      count = reduced ? 0 : Math.min(options.samples ?? 3, 4);
  ...
  function impactRipple(ctx, x, y, progress, options = {}) {
    ...
    const t = clamp(progress, 0, 1),
      radius = (options.radius || 46) * smoothstep(t);
    ctx.save();
  ...
  function focusTransition(camera, target, options = {}) {
    camera.focus(
      target,
