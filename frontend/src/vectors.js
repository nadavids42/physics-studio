...
export const PhysicsAnnotations = (() => {
  ...
  function scaleLegend(ctx, s, specs, x = 12, y = 12) {
    const physical = (specs || []).filter(
      (spec) =>
        spec.scale_mode === "physical" &&
        Number.isFinite(spec.pixels_per_unit) &&
        spec.pixels_per_unit > 0 &&
        spec.units,
    );
    if (!physical.length) return;
    const scale = physical[0].pixels_per_unit,
      units = physical[0].units;
    if (
      physical.some(
    ...
    const target = 40 / scale,
      ...
      quantity = units === "N" ? "Force" : "Vector",
      text = `${quantity} scale (linear): ${reference} ${units}`;
    ctx.save();
    V.applyText(ctx, s, "helper");
    const pad = 8,
      boxWidth = Math.min(
