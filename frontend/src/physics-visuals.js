const token = (state, group, name, fallback) =>
  state.config.visualTokens?.[group]?.[name] ?? fallback;
...
function applyText(context, state, role = "label") {
  ...
  const weight =
    role === "heading"
      ? type.weight_semibold || 650
      : role === "label"
        ? type.weight_medium || 550
        : type.weight_regular || 400;
...
function background(context, state, options = {}) {
  ...
  context.fillStyle =
    options.color || token(state, "colors", "canvas", "#F7FAFC");
...
function roundedPanel(context, state, x, y, width, height, options = {}) {
  ...
  context.fill();
  context.strokeStyle =
    options.stroke || token(state, "colors", "border", "#B8C5D1");
  context.lineWidth =
    options.lineWidth || token(state, "shape", "line_regular", 1.5);
  context.stroke();
}
...
function arrow(context, state, from, to, options = {}) {
  const color =
    options.color ||
    token(state, "colors", options.role || "net_force", "#C2410C");
...
function grid(context, state, box, options = {}) {
  ...
  context.save();
  context.strokeStyle =
    options.color || token(state, "colors", "grid", "#526577");
  context.globalAlpha =
    options.opacity ?? token(state, "shape", "grid_opacity", 0.16);
...
function trail(context, state, points, map, options = {}) {
  ...
  context.save();
  context.strokeStyle =
    options.color || token(state, "colors", "trajectory", "#1769AA");
  context.lineWidth =
    options.width || token(state, "shape", "line_regular", 1.5);
  context.lineCap = "round";
  context.globalAlpha =
    options.opacity ?? token(state, "shape", "trail_opacity", 0.24);
  if (options.dashed) context.setLineDash([7, 5]);
  context.beginPath();
