...
from physics_playground.canvas.ray_diagram import SCENE as RAY_SCENE,build_ray_diagram
from physics_playground.canvas.vector_diagram import SCENE as VECTOR_SCENE,build_vector_direction_document
from physics_playground.canvas.vector_field import SCENE as FIELD_SCENE,build_vector_field_document
from physics_playground.subjects.light_and_electricity.magnetic_forces.physics import ForceDirection,ForceMode,MagneticForceParameters,simulate as simulate_magnetic
from physics_playground.subjects.light_and_electricity.reflection_refraction.physics import ReflectionRefractionParameters,simulate as simulate_optics
...
def test_optics_scene_uses_medium_regions_boundary_normal_angles_and_scale_disclosure():
    for token in ("Medium 1","Medium 2","normalLine","angleArc","totalInternalReflection","displayed ray lengths are schematic"):
        assert token in RAY_SCENE
    result=simulate_optics(ReflectionRefractionParameters(60,1.5,1))
    document=build_ray_diagram(rays=[ray.to_dict() for ray in result.rays],message=result.outcome,seed=4,interface=True,
        medium_1=1.5,medium_2=1,incident_angle_deg=60,refraction_angle_deg=None,total_internal_reflection=True)
...
def test_magnetic_model_baselines_cover_charge_wire_reversal_and_zero_force():
    ...
    wire=simulate_magnetic(MagneticForceParameters(mode=ForceMode.CURRENT_WIRE,current_a=2,wire_length_m=.5,magnetic_field_t=.2,field_angle_deg=90))
    assert positive.force_z_n==pytest.approx(2e-5) and positive.direction is ForceDirection.OUT_OF_SCREEN
    assert negative.force_z_n==pytest.approx(-2e-5) and negative.direction is ForceDirection.INTO_SCREEN
    assert parallel.force_z_n==0 and parallel.direction is ForceDirection.ZERO
    assert wire.force_z_n==pytest.approx(.2)
...
def test_magnetic_scene_uses_typed_normalized_vectors_subject_assets_and_no_false_force_arrow():
    ...
    document=build_vector_direction_document(motion=(1,0),field=(1,0),force_z=0,motion_label="Velocity v",message="Zero",seed=2)
...
def test_electric_field_render_inputs_are_precomputed_bounded_and_deterministic():
    samples=[{"x":-1,"y":0,"ex":None,"ey":None,"v":None},{"x":0,"y":0,"ex":3.,"ey":4.,"v":-2.},{"x":1,"y":0,"ex":6.,"ey":8.,"v":5.}]
    kwargs=dict(samples=samples,charges=[{"q":1e-6,"x":0,"y":0}],grid_size=3,extent=1,test_x=.5,test_y=.5,test_charge=-1e-9,message="Field",seed=8)
    first=build_vector_field_document(**kwargs);second=build_vector_field_document(**kwargs)
    assert first==second
...
def test_electric_field_scene_uses_semantic_assets_grid_exclusions_legend_and_disclosure():
    for token in ("PhysicsAssets.grid","PhysicsAssets.charge","PhysicsAnnotations.dimensionLine","Hatched: singularity excluded",
                  "normalized for visibility using logarithmic magnitude","PhysicsVisuals.responsive"):
...
def test_wave1_pages_use_the_shared_chart_system_without_native_line_charts():
    ...
    from physics_playground.subjects.light_and_electricity.magnetic_forces import page as magnetic_page
    from physics_playground.subjects.light_and_electricity.reflection_refraction import page as optics_page
