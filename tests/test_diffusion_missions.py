...
from physics_playground.subjects.fluids_and_matter.diffusion.physics import DiffusionParameters,simulate
...
def test_diffusion_evaluator_returns_structured_mission_evidence() -> None:
    ...
    assert {item.mission_id for item in evaluations}=={"diffusion_unbiased","diffusion_two_d","diffusion_bias","diffusion_compare"}
...
def test_diffusion_comparison_and_bias_require_their_actual_runs() -> None:
    ...
    result=simulate(DiffusionParameters(bias_x=.3))
