...
from physics_playground.expansion_catalog import EXPANSION_MANIFESTS,parameter_type,resolve_entrypoint
...
from physics_playground.serialization import dataclass_from_dict,dumps,to_jsonable

EXPANSION_IDS={"inclined_plane","torque_levers","center_of_mass","roller_coaster","rotational_motion","wave_interference","doppler_effect","reflection_refraction","thin_lenses","electric_fields","magnetic_forces","buoyancy","fluid_pressure","gas_laws","diffusion"}
...
def test_every_expansion_is_registered_reachable_and_contract_complete() -> None:
    ...
    for manifest in EXPANSION_MANIFESTS:
        validate_expansion_definition(manifest);definition=SIMULATIONS_BY_ID[manifest.metadata.id]
...
def test_parameter_and_result_models_are_typed_dataclasses() -> None:
    for manifest in EXPANSION_MANIFESTS:
        model=parameter_type(manifest);assert dataclasses.is_dataclass(model)
...
def test_all_parameter_models_round_trip_through_notebook_json() -> None:
    ...
    for manifest in EXPANSION_MANIFESTS:
        original=parameter_type(manifest)();payload=to_jsonable(original)
        restored=dataclass_from_dict(type(original),payload)
        assert restored==original
        notebook.add_trial(simulation_id=manifest.metadata.id,parameters=payload,prediction=None,result_summary="audit",metrics={"value":1.},earned_badges=(),random_seed=1,model_version=manifest.metadata.model_version)
    decoded=json.loads(notebook.to_json());restored_notebook=ExperimentNotebook.from_dict(decoded)
...
def test_pages_expose_all_modes_notebook_assumptions_and_safe_keys() -> None:
    ...
    seen_mode_keys=set()
    for manifest in EXPANSION_MANIFESTS:
        spec=util.find_spec(manifest.page_entrypoint.rsplit(".",1)[0]);assert spec and spec.origin;source=Path(spec.origin).read_text()
        ...
        marker='mode_navigation(key="';start=source.index(marker)+len(marker);key=source[start:source.index('"',start)]
        assert key not in seen_mode_keys;seen_mode_keys.add(key)
...
def test_specialized_fluid_manifests_match_runtime_renderers() -> None:
    ...
    assert by_id["diffusion"].canvas_entrypoint.endswith("diffusion_player.build_diffusion_document")
