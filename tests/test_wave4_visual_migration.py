...
from physics_playground.canvas.boing import SCENE as BOING_SCENE, build_boing_canvas
from physics_playground.models.spring import SpringParameters, simulate_spring
from physics_playground.subjects.mechanics.canvas import SCENE as MECHANICS_SCENE, document
from physics_playground.subjects.mechanics.roller_coaster.physics import RollerCoasterParameters, simulate
...
def test_track_is_a_shared_scientific_asset_and_coaster_uses_it():
    ...
    for token in ("PhysicsAssets.track", "PhysicsAssets.cart", "PhysicsAssets.ruler",
                  "PhysicsAnnotations.motionDirection", "Unreachable with"):
...
def test_coaster_document_uses_model_track_distance_height_and_speed_samples():
    result = simulate(RollerCoasterParameters(initial_height_m=24, hill_height_m=14,
                                               final_height_m=3, track_length_m=80))
    normalized = [distance / result.parameters.track_length_m for distance in result.distance_m]
    payload = document("coaster", [{"id": "car", "x": normalized, "y": list(result.speed_m_s)}],
                       message=result.outcome, seed=9,
                       scene_config={"trackLengthM": 80, "trackPoints": [
                           {"distance": 0, "height": 24}, {"distance": 40, "height": 14},
                           {"distance": 80, "height": 3}], "reachableDistanceM": result.distance_m[-1]})
...
def test_unreachable_coaster_samples_stop_before_the_unreachable_point():
    result = simulate(RollerCoasterParameters(initial_height_m=5, hill_height_m=30,
                                               final_height_m=0, initial_speed_m_s=0))
...
def test_lossy_coaster_mechanical_energy_decreases_by_modeled_loss():
    ...
    for distance, energy in zip(result.distance_m, result.mechanical_energy_j):
...
def test_boing_uses_recorded_position_and_velocity_without_phase_changes():
    result = simulate_spring(SpringParameters(2, 20, 1, .4, 3, 1, 10, 180))
...
def test_boing_shared_assets_references_and_vector_semantics():
    for token in ("PhysicsAssets.spring", "PhysicsAssets.mass", "PhysicsAnnotations.pathGuide",
                  "PhysicsAnnotations.velocityTrail", "PhysicsAnnotations.vectorSet",
                  "PhysicsAnnotations.dimensionLine", "damping envelope",
                  "scale_mode:'physical'", "scale_mode:'normalized'", "restoring=-"):
...
def test_wave4_analysis_pages_use_accessible_shared_chart_rendering():
    ...
    assert "series_figure(" in coaster and "render_chart" in coaster and "st.line_chart" not in coaster
