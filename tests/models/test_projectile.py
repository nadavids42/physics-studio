...
from physics_playground.models.projectile import ProjectileParameters, simulate_no_drag, simulate_quadratic_drag


@pytest.mark.parametrize("speed,angle,gravity",[(20.0,30.0,9.81),(35.0,45.0,9.81),(18.0,60.0,1.62)])
...
def test_maximum_level_ground_range_is_approximately_45_degrees()->None:
    results={angle:simulate_no_drag(ProjectileParameters(25.0,angle,9.81)).range_m for angle in range(5,90)}
...
def test_drag_ground_impact_is_interpolated_to_exact_zero()->None:
    result=simulate_quadratic_drag(ProjectileParameters(30,40,9.81,drag_coefficient_kg_m=.08,time_step_s=.02))
...
def test_drag_reduces_range_and_mechanical_energy()->None:
    ...
    drag=simulate_quadratic_drag(ProjectileParameters(30,40,9.81,drag_coefficient_kg_m=.08))
...
def test_non_landing_limit_returns_clear_failure()->None:
    result=simulate_quadratic_drag(ProjectileParameters(30,80,1.62,drag_coefficient_kg_m=.01,max_time_s=.1))
