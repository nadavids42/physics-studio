...
from physics_playground.visual.tokens import DARK_THEME,LIGHT_THEME,MOTION,RESPONSIVE,SHAPE,SPACING,TYPOGRAPHY,theme_payload
...
def test_design_token_groups_are_available_and_palette_shapes_match():
    ...
    assert [field.name for field in fields(LIGHT_THEME)]==[field.name for field in fields(DARK_THEME)]
...
def test_shared_asset_configuration_semantic_golden_snapshot():
    assets=(
        AssetSpec(AssetKind.BLOCK,20,30,width=60,height=40,label="Block A",style=AssetStyle(selected=True)),
        AssetSpec(AssetKind.FORCE_ARROW,20,30,width=80,label="Net force",style=AssetStyle(shadow=False),options={"scale_mode":"schematic"}),
    )
    canonical=json.dumps([asset.to_dict() for asset in assets],sort_keys=True,separators=(",",":"))
    assert hashlib.sha256(canonical.encode()).hexdigest()=="60a597853f53bda99ecfb60d2ad5c693d2013f47e39a903d1705d121e4a29e5c"
...
def test_vector_display_geometry_for_all_scaling_modes():
    ...
    normalized=VectorSpec(0,0,30,40,"velocity",scale_mode=VectorScaleMode.NORMALIZED,fixed_length_px=72)
    schematic=VectorSpec(0,0,.1,.1,"net_force",scale_mode=VectorScaleMode.SCHEMATIC,fixed_length_px=48)
...
def test_reduced_motion_disables_autoplay_and_decorative_motion_only():
    document=build_player_document(config={"durationMs":1000,"autoplay":True,"tracks":[],"events":[]},scene_javascript="const scene={draw(){}};",
        logical_width=200,logical_height=100,accessible_label="Reduced motion",idle_hint="Play")
...
def test_theme_switching_is_serialized_into_deterministic_player_input(theme):
    ...
    kwargs=dict(scene_javascript="const scene={draw(){}};",logical_width=100,logical_height=50,accessible_label="Theme",idle_hint="Play")
    first=build_player_document(config=config,**kwargs);second=build_player_document(config=config,**kwargs)
    assert first==second
...
def test_visual_preferences_round_trip_and_invalid_values_fall_back():
    ...
    assert VisualPreferences.from_dict({"presentation_level":"unknown","theme":"unknown"})==VisualPreferences()
...
def test_responsive_layout_contract_has_ordered_breakpoints_and_canvas_helper():
    ...
    assert "img,svg,canvas,iframe {max-width:100%;}" in build_player_document(config={"durationMs":1,"tracks":[],"events":[]},
        scene_javascript="const scene={draw(){}};",logical_width=100,logical_height=50,accessible_label="Responsive",idle_hint="Play")


def _existing_session_keys()->set[str]:
    root=Path(__file__).parents[1]/"physics_playground";keys=set()
    for path in root.rglob("*.py"):
        if path.name=="accessibility.py" and path.parent.name=="presentation":continue
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node,ast.Call):
                for keyword in node.keywords:
                    if keyword.arg=="key" and isinstance(keyword.value,ast.Constant) and isinstance(keyword.value.value,str):keys.add(keyword.value.value)
    return keys
...
def test_pilot_scene_semantic_golden_signatures():
    ...
    signatures={
        "projectile":("PhysicsAssets.cannon","PhysicsAssets.projectile","PhysicsAnimation.fadingTrail"),
        ...
        "collision":("PhysicsAssets.cart","PhysicsAnimation.impactRipple","PhysicsAnimation.collisionFlash"),
        "optics":("PhysicsAssets.lens","PhysicsAssets.ray","PhysicsAnnotations.normalLine"),
    }
    sources={"projectile":cannonball.SCENE,"pendulum":pendulum.SCENE,"orbit":orbit.SCENE,
             "collision":bumper_cars.SCENE_JAVASCRIPT,"optics":ray_diagram.SCENE}
    actual={name:tuple(token for token in expected if token in sources[name]) for name,expected in signatures.items()}
