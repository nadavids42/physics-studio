...
from physics_playground.canvas.double_pendulum import SCENE as DOUBLE_SCENE, build_double_canvas
from physics_playground.canvas.earth_tunnel import SCENE as TUNNEL_SCENE, build_tunnel_canvas, build_tunnel_comparison_canvas
from physics_playground.models.double_pendulum import DoublePendulumParameters, simulate_double_pendulum
from physics_playground.models.earth_tunnel import (TunnelModel, TunnelParameters, simulate_tunnel,
                                                       uniform_acceleration)


def test_double_pendulum_uses_shared_assets_bounded_trails_and_accessible_identity():
    for token in ("PhysicsAssets.pivot", "PhysicsAssets.rod", "PhysicsAssets.pendulumBob",
                  "PhysicsAnnotations.velocityTrail", "Baseline A (solid)", "Perturbed B (dashed)"):
        assert token in DOUBLE_SCENE or token in build_double_canvas(
            simulate_double_pendulum(DoublePendulumParameters(duration_s=.1)), seed=1, autoplay=False)
    result = simulate_double_pendulum(DoublePendulumParameters(duration_s=.1))
...
def test_double_joint_and_bob_payloads_are_the_recorded_cartesian_tracks():
    result = simulate_double_pendulum(DoublePendulumParameters(duration_s=.1, time_step_s=.005))
...
def test_double_separation_uses_track_coordinates_and_camera_is_fixed():
    result = simulate_double_pendulum(DoublePendulumParameters(duration_s=.1))
    payload = build_double_canvas(result, seed=3, autoplay=False, show_separation=True,
                                  inspect_system="b")
...
def test_tunnel_uses_shared_planet_mass_measurements_trail_and_normalized_acceleration():
    for token in ("PhysicsAssets.planet", "PhysicsAssets.mass", "PhysicsAnnotations.dimensionLine",
                  "PhysicsAnnotations.centerOfMass", "PhysicsAnnotations.velocityTrail",
                  "role:'acceleration'", "scale_mode:'normalized'", "Acceleration direction is physical"):
        assert token in TUNNEL_SCENE


def test_tunnel_position_acceleration_and_turning_points_use_recorded_values():
    parameters = TunnelParameters(6_371_000, 9.81, .5)
    ...
    acceleration = next(plot for plot in result.plots if plot.id == "acceleration_position").series[0].y
    ...
    assert min(position) == pytest.approx(-parameters.radius_m * parameters.start_fraction / 1000, rel=2e-5)
...
def test_tunnel_comparison_retains_one_synchronized_player_timeline_and_noncolor_identity():
    ...
    payload = build_tunnel_comparison_canvas((("Uniform", uniform, ""), ("Radial", radial, "")),
                                              seed=5, autoplay=False)
