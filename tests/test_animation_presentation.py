...
def _document(config=None):
    return build_player_document(config=config or {"durationMs":1000,"tracks":[{"id":"x","label":"x","x":[0,1,2]}],"events":[]},
        scene_javascript="const scene={draw(){}};",logical_width=640,logical_height=320,
        accessible_label="Animation presentation",idle_hint="Play")


def test_player_has_pause_resume_restart_stepping_speed_and_scrubbing_controls():
    document=_document()
    for token in ('id="play-pause"','id="replay"','id="step-back"','id="step-forward"',
                  'id="speed"','id="scrubber"',"resume()", "stepFrames(amount)"):
...
def test_player_cleans_up_browser_resources_and_does_not_add_stationary_trails():
    for token in ("resizeObserver?.disconnect", "removeEventListener('visibilitychange'",
                  "removeEventListener('keydown'", "cancelAnimationFrame"):
...
def test_animation_helpers_cover_camera_trails_blur_impacts_and_focus():
    for name in ("Camera","withCamera","fadingTrail","subtleMotionBlur","impactRipple",
                 "collisionFlash","focusTransition"):
...
def test_building_animation_document_does_not_mutate_physics_payload():
    config={"durationMs":1000,"tracks":[{"id":"body","label":"Body","x":[0.,1.,2.],"y":[3.,4.,5.]}],
            "events":[{"id":"impact","fraction":.5}],"view":{"minimum":0,"maximum":2}}
