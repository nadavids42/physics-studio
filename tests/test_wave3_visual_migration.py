...
from physics_playground.subjects.mechanics.center_of_mass.physics import CenterOfMassParameters, simulate as simulate_center
from physics_playground.subjects.mechanics.inclined_plane.physics import InclinedPlaneParameters, simulate as simulate_incline
from physics_playground.subjects.mechanics.rotational_motion.physics import RotationalParameters, simulate as simulate_rotation
from physics_playground.subjects.mechanics.torque_levers.physics import LeverParameters, simulate as simulate_lever


def test_incline_scene_uses_shared_geometry_and_declares_vector_semantics():
    for token in ("PhysicsAssets.ramp", "PhysicsAssets.block", "PhysicsAnnotations.normalLine",
                  "PhysicsAnnotations.angleArc", "PhysicsAnnotations.pathGuide",
                  "PhysicsAnnotations.vectorSet", "scale_mode"):
        assert token in SCENE
    result = simulate_incline(InclinedPlaneParameters(angle_deg=30, static_friction=.9, kinetic_friction=.4))
    assert result.acceleration_m_s2 == 0
    payload = document("ramp", [{"id": "block", "x": [0, 0]}], message=result.outcome, seed=3,
...
def test_incline_force_components_and_friction_direction_remain_physical():
    result = simulate_incline(InclinedPlaneParameters(mass_kg=2, angle_deg=40, static_friction=.2, kinetic_friction=.1))
...
def test_lever_scene_has_distinct_force_dimension_and_torque_annotations():
    for token in ("PhysicsAssets.rod", "PhysicsAssets.pivot", "PhysicsAssets.block",
                  "PhysicsAnnotations.dimensionLine", "PhysicsAnnotations.centerOfMass",
                  "PhysicsAnnotations.vectorSet", "PhysicsAssets.torqueArc"):
        assert token in SCENE
    balanced = simulate_lever(LeverParameters(load_force_n=100, load_arm_m=1, effort_force_n=50, effort_arm_m=2))
...
def test_center_marker_and_bounded_illustrative_radius_contract():
    ...
    for token in ("PhysicsAssets.mass", "PhysicsAssets.ruler", "PhysicsAnnotations.centerOfMass",
                  "Math.max(11,Math.min(24", "Mass radii are bounded illustrations"):
        assert token in SCENE
...
def test_rotation_orientation_uses_recorded_angle_but_is_visually_bounded():
    ...
    payload = document("rotation", [{"id": "body", "x": [0, result.angular_position_rad[-1]]}],
                       message=result.outcome, seed=4, scene_config={"torqueNm": 10})
...
def test_wave3_pages_use_shared_charts_and_separate_rotation_units():
    ...
    sources = [(root / simulation / "page.py").read_text(encoding="utf-8")
