...
from physics_playground.canvas.fluid_container import SCENE as FLUID_SCENE
from physics_playground.canvas.fluid_container import build_fluid_document
from physics_playground.canvas.gas_container import SCENE as GAS_SCENE
...
from physics_playground.subjects.fluids_and_matter.gas_laws.physics import (
    simulate as simulate_gas,
)


def test_buoyancy_numerical_states_and_apparent_weight_are_preserved():
    floating = simulate_buoyancy(
        BuoyancyParameters(object_density_kg_m3=600, fluid_density_kg_m3=1000)
    )
    neutral = simulate_buoyancy(
        BuoyancyParameters(object_density_kg_m3=1000, fluid_density_kg_m3=1000)
    )
    sinking = simulate_buoyancy(
        BuoyancyParameters(object_density_kg_m3=1500, fluid_density_kg_m3=1000)
    )
    assert (
        floating.state is BuoyancyState.FLOATING
        and floating.submerged_fraction == pytest.approx(0.6)
    )
...
def test_buoyancy_scene_uses_shared_assets_measurements_and_declared_force_semantics():
    for token in (
        "PhysicsAssets.fluidContainer",
        "PhysicsAssets.fluidSurface",
        "PhysicsAssets.block",
    ...
    document = build_fluid_document(
        kind="buoyancy",
        message="Sinking",
        seed=1,
        submerged_fraction=1,
        state="Sinking",
        object_density=1500,
        fluid_density=1000,
...
def test_pressure_numerical_samples_and_absolute_offset_are_preserved():
    ...
    assert [sample.depth_m for sample in result.samples] == pytest.approx(
...
def test_pressure_scene_maps_selected_depth_and_uses_normalized_pressure_arrows():
    for token in (
        ...
        "Pressure-arrow lengths are normalized",
        "Surface:",
        "Gauge:",
        "Absolute:",
    ):
    ...
    document = build_fluid_document(
        ...
        depth=5,
        maximum_depth=10,
        gauge_pressure=49050,
        absolute_pressure=150375,
        surface_pressure=101325,
        pressure_samples=[
            {"depth": 0, "gauge": 0},
            {"depth": 5, "gauge": 49050},
            {"depth": 10, "gauge": 98100},
        ],
    )
...
def test_gas_state_values_are_unchanged_by_decorative_document_building():
    result = simulate_gas(GasLawParameters(GasLawScenario.BOYLE, target_volume_m3=0.012))
    before = (
        result.state_a.pressure_pa,
        result.state_a.volume_m3,
        result.state_a.temperature_k,
        result.state_b.pressure_pa,
        result.state_b.volume_m3,
        result.state_b.temperature_k,
    )
    build_gas_document(
...
def test_gas_particle_layout_is_precomputed_seeded_and_stable():
    first = build_gas_document(**_gas_kwargs(17))
    again = build_gas_document(**_gas_kwargs(17))
    different = build_gas_document(**_gas_kwargs(18))
...
def test_wave2_pages_use_shared_charts_and_separate_incompatible_buoyancy_units():
    ...
    sources = {
        module.__name__: open(module.__file__, encoding="utf-8").read()
        for module in (buoyancy_page, pressure_page, gas_page)
    }
