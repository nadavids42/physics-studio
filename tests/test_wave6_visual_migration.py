...
from physics_playground.canvas.scalar_field import SCENE as FIELD_SCENE
from physics_playground.canvas.scalar_field import build_scalar_field_document
from physics_playground.canvas.wavefronts import SCENE as WAVEFRONT_SCENE
...
from physics_playground.subjects.waves_and_oscillations.wave_interference.physics import (
    simulate as simulate_waves,
)


def test_resultant_is_exact_existing_pointwise_superposition():
    result = simulate_waves(
        WaveInterferenceParameters((WaveSource(1.2, 2, 1, 0.2), WaveSource(0.7, 3, 2 / 3, 1.1)))
    )
...
def test_wave_graph_plane_has_fixed_scale_phase_identity_and_measurements():
    ...
    payload = build_scalar_field_document(
        x=result.position_m,
        frames=result.superposition_frames,
        source_frames=result.source_frames,
        sources=({"amplitude": 1, "phaseDeg": 0}, {"amplitude": 1, "phaseDeg": 180}),
        measurements=({"index": 0},),
        time_s=result.time_s,
        interference_label="Destructive interference\nΔφ = 180°",
        duration_s=2,
        accessible_label="waves",
        completion_message=result.outcome,
        seed=1,
    )
...
def test_doppler_wavefront_payload_preserves_centers_radii_source_and_observer():
    result = simulate_doppler(
        DopplerParameters(source_velocity_m_s=30, observer_velocity_m_s=-10, samples=9)
    )
    frames = [
        {
            "source": frame.source_position_m,
            "observer": frame.observer_position_m,
            "centers": frame.centers_m,
            "radii": frame.radii_m,
        }
        for frame in result.frames
    ]
    payload = build_wavefront_document(
        frames=frames,
        world_min=-700,
        world_max=800,
        duration_s=2,
        ...
        observer_velocity_m_s=-10,
        wavelength_ahead_m=result.wavelength_ahead_m,
        wavelength_behind_m=result.wavelength_behind_m,
        motion_label=result.motion.value,
    )
...
def test_doppler_shared_assets_physical_geometry_and_motion_semantics():
    for token in (
        "PhysicsAssets.source",
        "PhysicsAssets.observer",
        "PhysicsAssets.wavefront",
        "PhysicsAssets.ruler",
        "scale_mode:'normalized'",
        "source ${c.sourceVelocityMps",
        "observer ${c.observerVelocityMps",
        "line thickness is decorative",
    ):
