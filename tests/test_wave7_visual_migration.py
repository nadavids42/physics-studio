...
from physics_playground.canvas.diffusion_player import (
    SCENE,
...
def test_path_drawing_work_is_bounded_and_trails_fade():
    ...
    payload = build_diffusion_document(
        paths=(tuple((float(i), 0.0) for i in range(500)),),
        dimensions=1,
        extent=500,
        message="done",
        seed=2,
...
def test_distribution_bins_are_counts_over_meter_widths():
    centers, counts, width = distribution_bins((-1.0, -0.9, 0.0, 0.1, 0.9, 1.0), bin_count=4)
    assert len(centers) == len(counts) == 4
    assert sum(counts) == 6
    assert width == pytest.approx(0.5)
    assert centers == pytest.approx((-0.75, -0.25, 0.25, 0.75))
...
def test_theoretical_msd_has_square_meter_units_and_correct_dimension_factor():
    ...
    page = (
        ...
        / "physics_playground/subjects/fluids_and_matter/diffusion/page.py"
    ).read_text()
...
def test_trajectory_identity_uses_numbers_line_styles_and_markers_not_only_color():
    for token in ("dashes=[", "T${j+1}", "'dashed':'solid'", "ctx.rect(ox-3", "PhysicsAssets.mass"):
...
def test_diffusion_charts_use_binned_histogram_and_shared_msd_system():
    ...
    assert "axis.bar(centers, counts, width=width * 0.9" in source
