...
from physics_playground.models.expansion import (
    ExpansionDefinition, REQUIRED_MODE_REQUIREMENTS, SubjectArea, TestRequirements,
)
...
def mission(number: int) -> MissionDefinition:
    return MissionDefinition(f"sample_{number}", "sample", f"Mission {number}", "Try it", ("Hint",),
                             MissionType.DISCOVERY, Difficulty.BEGINNER, (), False,
                             f"run:sample_{number}", "Sample")


def valid_definition() -> ExpansionDefinition:
    metadata = SimulationDefinition("sample", "Sample", "🧪", "Description", "physics_playground.pages.sample",
                                    "Sample", tuple(item.mode for item in REQUIRED_MODE_REQUIREMENTS),
                                    "What changes?", ("Testing",), Difficulty.BEGINNER, 3,
                                    "shared-browser-player", "sample-1.0")
    return ExpansionDefinition(metadata, SubjectArea.MECHANICS, "SampleParameters", "SampleResult",
                               ...
                               REQUIRED_MODE_REQUIREMENTS, tuple(mission(i) for i in range(3)),
                               (ModelAssumption("ideal", "Ideal model"),), ("Ignores losses",))
...
def test_all_four_subject_areas_are_stable() -> None:
    assert {item.value for item in SubjectArea} == {
        "mechanics", "waves_and_oscillations", "light_and_electricity", "fluids_and_matter"
    }
...
def test_insufficient_boundary_tests_are_rejected() -> None:
    definition = valid_definition()
    with pytest.raises(PhysicsValidationError):
        validate_expansion_definition(replace(definition, tests=TestRequirements(validation_tests=2)))
