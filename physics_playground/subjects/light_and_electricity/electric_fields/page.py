...
def controls(prefix="field"):
    ...
    for i in range(count):
        with st.expander(f"Charge {i+1}",expanded=i<2):
            c=st.columns(3);x=c[0].slider("x position (m)",-3.,3.,defaults[i][0],.1,key=f"{prefix}_x{i}");y=c[1].slider("y position (m)",-3.,3.,defaults[i][1],.1,key=f"{prefix}_y{i}");micro=c[2].slider("Charge (µC)",-5.,5.,defaults[i][2],.1,key=f"{prefix}_q{i}");
            if micro==0:st.warning("A source charge cannot be zero; choose a positive or negative value.")
            charges.append(PointCharge(micro*1e-6,x,y))
    c1,c2,c3=st.columns(3);tx=c1.slider("Test-charge x (m)",-3.5,3.5,0.,.1,key=f"{prefix}_tx");ty=c2.slider("Test-charge y (m)",-3.5,3.5,1.,.1,key=f"{prefix}_ty");test_micro=c3.slider("Test charge (µC)",-2.,2.,.001,.001,key=f"{prefix}_tq");grid=st.select_slider("Field-grid density",options=[17,21,25,29,33,37,41,45,49],value=25,key=f"{prefix}_grid");st.caption(f"Performance budget: {grid**2:,} of {MAX_FIELD_POINTS:,} maximum field points. Points within 0.12 m of a source are excluded.");return ElectricFieldParameters(tuple(charges),test_micro*1e-6,tx,ty,4.,grid,.12)
def explore():
    mode_heading(LearningMode.EXPLORE,"Arrange charges and probe the field");r=simulate(controls());c=st.columns(3);c[0].metric("Field at test charge",f"{r.test_field_magnitude_n_c:.3g} N/C");c[1].metric("Potential",f"{r.test_potential_v:.3g} V");c[2].metric("Force",f"{r.force_magnitude_n:.3g} N");st.caption("Text outcome: "+r.outcome);diagram(r,20262901);obs=st.text_input("Optional notebook observation",key="field_obs")
    if st.button("⚡ Map electric field",type="primary",use_container_width=True):record(r,20262901,obs,badges=kidtools.process_run(ID,evaluate(r)));st.rerun()
    kidtools.mission_checklist("Electric Fields")
def compare():
    mode_heading(LearningMode.COMPARE,"Like charges versus a dipole");a=simulate(ElectricFieldParameters((PointCharge(1e-6,-1,0),PointCharge(1e-6,1,0)),test_y_m=1));b=simulate(ElectricFieldParameters((PointCharge(1e-6,-1,0),PointCharge(-1e-6,1,0)),test_y_m=1));changed_variable_banner(ChangedVariable("Right charge","+1 µC","−1 µC"));c=st.columns(2);c[0].metric("Like-charge field",f"{a.test_field_magnitude_n_c:.2f} N/C");c[1].metric("Dipole field",f"{b.test_field_magnitude_n_c:.2f} N/C")
    if st.button("▶ Run field comparison",use_container_width=True):
        for label,r,seed in (("Run A",a,20262911),("Run B",b,20262912)):record(r,seed,"Changing one sign reshapes both field and potential",label,kidtools.process_run(ID,evaluate(r,True)))
    diagram(b,20262912)
def analyze():
    mode_heading(LearningMode.ANALYZE,"Field and potential along a line");r=simulate(controls("field_analysis"));xs=[-4+i*.04 for i in range(201)];potential=[];magnitude=[]
    for x in xs:
...
def render():
    st.header("⚡ Electric Fields");revealed=kidtools.prediction_quiz(key="field_quiz",question="Which direction does the electric field point near a positive source charge?",options=["Away from it","Toward it","Clockwise around it"],correct_index=0,reveal_text="A positive test charge would be pushed away, which defines the field direction.",mission_id="field_predict")
    if not revealed:return
    mode=mode_navigation(key="field_mode");{LearningMode.EXPLORE:explore,LearningMode.COMPARE:compare,LearningMode.ANALYZE:analyze,LearningMode.MODEL:model}[mode]()
