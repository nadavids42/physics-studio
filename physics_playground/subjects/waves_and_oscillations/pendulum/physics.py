...
class PendulumModel(StrEnum):
    SMALL_ANGLE = "Small-angle approximation"
...
class PendulumParameters:
    ...
    def validate(self):
        ...
        require_positive("Rope length", self.length_m)
        require_positive("Gravity", self.gravity_m_s2)
        require_positive("Duration", self.duration_periods)
        if not 0 < abs(self.release_angle_deg) < 179:
            raise PhysicsValidationError("Release angle must be between 0° and 179°.")
        if self.samples < 4:
            raise PhysicsValidationError("Samples must be at least 4.")
        enforce_budget("Pendulum samples", self.samples, MAX_TRAJECTORY_SAMPLES)

    @property
    def small_angle_period_s(self):
        return 2 * math.pi * math.sqrt(self.length_m / self.gravity_m_s2)
...
def nonlinear_period_series(length: float, gravity: float, angle_deg: float) -> float:
    ...
    ratio = 1 + t * t / 16 + 11 * t**4 / 3072 + 173 * t**6 / 737280
    return 2 * math.pi * math.sqrt(length / gravity) * ratio


def _result(p, t, theta, omega, period, method):
    x = p.length_m * np.sin(theta)
    y = -p.length_m * np.cos(theta)
    speed = np.abs(p.length_m * omega)
    energy = 0.5 * (p.length_m * omega) ** 2 + p.gravity_m_s2 * p.length_m * (1 - np.cos(theta))
    ...
    plots = (
        PlotData(
            ...
            "Time (s)",
            "Angle (degrees)",
            (PlotSeries("angle", method, tuple(t), tuple(np.degrees(theta))),),
        ),
        PlotData(
            "angular_velocity_time",
            "Angular velocity versus time",
            "Time (s)",
            "Angular velocity (rad/s)",
            (PlotSeries("omega", "ω(t)", tuple(t), tuple(omega)),),
        ),
        PlotData(
            "energy_time",
        ...
        PlotData(
            ...
            "Angle (rad)",
            "Angular velocity (rad/s)",
            (PlotSeries("phase", "(θ,ω)", tuple(theta), tuple(omega)),),
            PlotKind.PHASE,
        ),
    )
    return PendulumResult(
        ...
        parameters=p,
        metrics=(
            SummaryMetric("period", "Period", period, "s", f"{period:.2f} s"),
            SummaryMetric(
                "maximum_speed",
                "Maximum bob speed",
                float(np.max(speed)),
                "m/s",
                f"{np.max(speed):.2f} m/s",
            ),
            SummaryMetric("energy_drift", "Energy drift", drift * 100, "%", f"{drift * 100:.4f}%"),
        ),
        events=(
            SimulationEvent("release", EventKind.START, 0, "Bob released"),
            SimulationEvent(
                "complete", EventKind.COMPLETION, float(t[-1]), "Pendulum trial complete"
            ),
        ),
        plots=plots,
        animation=AnimationData(
            ...
            tuple(t),
            (AnimationTrack("bob", "Pendulum bob", tuple(x), tuple(y), {"color": "#66BB6A"}),),
            4800,
            {"length": p.length_m, "minimum": -p.length_m, "maximum": p.length_m},
        ),
        assumptions=(
            ModelAssumption("point_mass", "The bob is a point mass."),
            ModelAssumption("rigid_rope", "The rope is massless, rigid, and never stretches."),
            ModelAssumption("no_drag", "There is no air resistance or pivot friction."),
        ),
        period_s=period,
...
def simulate_small_angle(p: PendulumParameters) -> PendulumResult:
    p.validate()
    period = p.small_angle_period_s
    w0 = math.sqrt(p.gravity_m_s2 / p.length_m)
    t = np.linspace(0, p.duration_periods * period, p.samples)
    a = math.radians(p.release_angle_deg)
...
def simulate_nonlinear(p: PendulumParameters) -> PendulumResult:
    p.validate()
    period = nonlinear_period_series(p.length_m, p.gravity_m_s2, p.release_angle_deg)
    t = np.linspace(0, p.duration_periods * period, p.samples)
    dt = t[1] - t[0]
...
def simulate_pendulum(p: PendulumParameters) -> PendulumResult:
    return (
        simulate_small_angle(p) if p.model == PendulumModel.SMALL_ANGLE else simulate_nonlinear(p)
    )


def approximation_error_curve(length: float, gravity: float, max_angle: int = 85):
    angles = tuple(range(1, max_angle + 1))
    t0 = 2 * math.pi * math.sqrt(length / gravity)
    errors = tuple((nonlinear_period_series(length, gravity, a) / t0 - 1) * 100 for a in angles)
