...
def values(r):
    return {
        "source_frequency_hz": r.parameters.source_frequency_hz,
        "observed_frequency_hz": r.observed_frequency_hz,
        "frequency_shift_hz": r.frequency_shift_hz,
        "frequency_ratio": r.frequency_ratio,
        "wavelength_ahead_m": r.wavelength_ahead_m,
        "wavelength_behind_m": r.wavelength_behind_m,
    }
...
def animation(r, seed):
    ...
    frames = [
        {
            "source": f.source_position_m,
            "observer": f.observer_position_m,
            "centers": f.centers_m,
            "radii": f.radii_m,
        }
        for f in r.frames
    ...
    embed.show(
        build_wavefront_document(
            ...
            world_max=max(
                p.initial_observer_position_m + extent, p.initial_observer_position_m + 10
            ),
            duration_s=p.duration_s,
            message=r.outcome,
            seed=seed,
            source_velocity_m_s=p.source_velocity_m_s,
            observer_velocity_m_s=p.observer_velocity_m_s,
            wavelength_ahead_m=r.wavelength_ahead_m,
            wavelength_behind_m=r.wavelength_behind_m,
            motion_label=r.motion.value,
        ),
        height=450,
    )
...
def explore():
    mode_heading(LearningMode.EXPLORE, "Move the source and listener")
    r = simulate(controls())
    cols = st.columns(3)
    cols[0].metric("Emitted frequency", f"{r.parameters.source_frequency_hz:.1f} Hz")
    cols[1].metric(
        "Observed frequency", f"{r.observed_frequency_hz:.1f} Hz", f"{r.frequency_shift_hz:+.1f} Hz"
    )
...
def compare():
    mode_heading(LearningMode.COMPARE, "Approaching versus receding")
    a = simulate(DopplerParameters(source_velocity_m_s=60))
    b = simulate(DopplerParameters(source_velocity_m_s=-60))
    changed_variable_banner(
        ChangedVariable("Source direction", "Approaching at 60 m/s", "Receding at 60 m/s")
    )
    c = st.columns(2)
    c[0].metric("Approaching", f"{a.observed_frequency_hz:.1f} Hz")
    c[1].metric("Receding", f"{b.observed_frequency_hz:.1f} Hz")
    if st.button("▶ Run Doppler comparison", use_container_width=True):
        for label, r, seed in (("Run A", a, 20262611), ("Run B", b, 20262612)):
            record(
                r,
                seed,
...
def analyze():
    ...
    figure = series_figure(
        x=velocities,
        series={"Moving source": source, "Moving observer": observer},
        x_label="Velocity (m/s; right positive)",
        y_label="Observed frequency (Hz)",
        title="Doppler frequency versus motion",
    )
    render_chart(
        figure,
        "Positive source velocity approaches the observer and raises frequency; positive observer velocity recedes and lowers it. Source motion changes wavelength while observer motion changes encounter rate.",
    )
    plt.close(figure)


def model():
    mode_heading(LearningMode.MODEL, "The classical Doppler equation")
    st.latex(r"f_{obs}=f_s\frac{c-v_o}{c-v_s}")
    st.markdown(
        "Positions increase to the right, with the observer initially right of the source. Positive source velocity approaches the observer; positive observer velocity moves away. The wavelength ahead of the source is $(c-v_s)/f_s$."
    )
    assumptions_panel(
        (
            ModelAssumption("medium", "Sound travels through a stationary uniform medium"),
            ModelAssumption("subsonic", "Source and observer remain subsonic"),
        ),
        (
            "No wind or temperature gradients.",
            "No reflected sound or finite source size.",
            "Relativistic effects are not applicable to this sound model.",
        ),
    )
...
def render():
    ...
    revealed = mission_ui.prediction_quiz(
        ...
        correct_index=0,
        reveal_text="Approaching wavefronts are compressed, so they arrive more frequently.",
        mission_id="doppler_predict",
    )
    if not revealed:
        return
