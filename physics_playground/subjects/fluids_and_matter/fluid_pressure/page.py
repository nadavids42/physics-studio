...
def diagram(r,seed):legacy.show(build_fluid_document(kind="pressure",message=r.outcome,seed=seed,depth=r.parameters.depth_m,maximum_depth=r.parameters.maximum_depth_m,gauge_pressure=r.gauge_pressure_pa,absolute_pressure=r.absolute_pressure_pa,surface_pressure=r.parameters.surface_pressure_pa,fluid_density=r.parameters.fluid_density_kg_m3,pressure_samples=[{"depth":sample.depth_m,"gauge":sample.gauge_pressure_pa} for sample in r.samples]),height=570)
...
def compare():
    ...
    diagram(b,20263212)
def analyze():
    mode_heading(LearningMode.ANALYZE,"Compare pressure throughout a column");r=simulate(controls("pressure_analysis"));figure=series_figure(x=[s.depth_m for s in r.samples],series={"Gauge pressure":[s.gauge_pressure_pa/1000 for s in r.samples],"Absolute pressure":[s.absolute_pressure_pa/1000 for s in r.samples]},x_label="Depth (m)",y_label="Pressure (kPa)",title="Hydrostatic pressure through the column");render_chart(figure,"Both pressures increase linearly with depth; absolute pressure is gauge pressure shifted upward by surface pressure. Distinct line styles and markers identify the two series.");plt.close(figure)
def model():
