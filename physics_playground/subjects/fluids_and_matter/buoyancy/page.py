...
def diagram(r,seed):legacy.show(build_fluid_document(kind="buoyancy",message=r.outcome,seed=seed,submerged_fraction=r.submerged_fraction,state=r.state.value,object_density=r.effective_density_kg_m3,fluid_density=r.parameters.fluid_density_kg_m3,displaced_volume=r.displaced_volume_m3,weight=r.weight_n,buoyant_force=r.buoyant_force_n),height=570)
...
def compare():
    ...
    diagram(b,20263112)
def analyze():
    mode_heading(LearningMode.ANALYZE,"Density ratio and submerged fraction");fluid=st.slider("Analysis fluid density (kg/m³)",200.,2000.,1000.,25.);densities=list(range(100,2501,50));results=[simulate(BuoyancyParameters(object_density_kg_m3=d,fluid_density_kg_m3=fluid)) for d in densities];fraction_figure=series_figure(x=densities,series={"Submerged fraction":[r.submerged_fraction for r in results]},x_label="Object density (kg/m³)",y_label="Submerged fraction",title="Submerged fraction versus density");render_chart(fraction_figure,"Floating objects submerge in direct proportion to density ratio until they become fully submerged.");plt.close(fraction_figure);weight_figure=series_figure(x=densities,series={"Apparent weight":[r.apparent_weight_n for r in results]},x_label="Object density (kg/m³)",y_label="Apparent weight (N)",title="Apparent weight versus density");render_chart(weight_figure,"Objects denser than the fluid sink and retain positive apparent weight; floating objects have zero apparent weight in this static model.");plt.close(weight_figure)
def model():
