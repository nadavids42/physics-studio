...
def animation(r,seed):
    p=r.parameters;show(document("rotation",[{"id":"body","label":"Body","x":[0,r.angular_position_rad[-1]]}],message=r.outcome,seed=seed,autoplay=False,scene_config={"shape":BodyShape(p.shape).value,"torqueNm":p.torque_n_m,"omegaRadS":r.angular_velocity_rad_s[-1],"angularAccelerationRadS2":r.angular_acceleration_rad_s2}))
...
def analyze():
    mode_heading(LearningMode.ANALYZE,"Angular kinematics and energy");r=simulate(controls("rotation_analysis"));time=list(r.time_s)
    angle=series_figure(x=time,series={"Angular position":list(r.angular_position_rad)},x_label="Time (s)",y_label="Angle (rad)",title="Angular position");render_chart(angle,"Constant torque produces quadratically changing angular position.");plt.close(angle)
    speed=series_figure(x=time,series={"Angular velocity":list(r.angular_velocity_rad_s)},x_label="Time (s)",y_label="Angular velocity (rad/s)",title="Angular velocity");render_chart(speed,"Constant torque produces linearly changing angular velocity.");plt.close(speed)
    energy=series_figure(x=time,series={"Rotational energy":list(r.rotational_kinetic_energy_j),"Work done":list(r.work_done_j)},x_label="Time (s)",y_label="Energy (J)",title="Rotational work and energy");render_chart(energy,"Applied work equals the change in rotational kinetic energy relative to its initial value.");plt.close(energy)
def model():
