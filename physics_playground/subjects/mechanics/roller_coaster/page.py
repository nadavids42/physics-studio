...
def animation(r,seed):
    p=r.parameters;normalized=[distance/p.track_length_m for distance in r.distance_m]
    show(document("coaster",[{"id":"car","label":"Coaster","x":normalized or [0],"y":list(r.speed_m_s) or [0]}],message=r.outcome,seed=seed,autoplay=False,scene_config={"trackLengthM":p.track_length_m,"trackPoints":[{"distance":0,"height":p.initial_height_m},{"distance":p.track_length_m/2,"height":p.hill_height_m},{"distance":p.track_length_m,"height":p.final_height_m}],"reachableDistanceM":r.distance_m[-1] if r.distance_m else 0,"massKg":p.mass_kg,"gravityMps2":p.gravity_m_s2,"lossPerMeterJ":p.loss_per_meter_j}))
...
def analyze():
    mode_heading(LearningMode.ANALYZE,"Energy and speed along the track");r=simulate(controls("coaster_analysis"));distance=list(r.distance_m)
    energy=series_figure(x=distance,series={"Potential energy":list(r.potential_energy_j),"Kinetic energy":list(r.kinetic_energy_j),"Mechanical energy":list(r.mechanical_energy_j)},x_label="Distance along track (m)",y_label="Energy (J)",title="Energy along the modeled track");render_chart(energy,"Potential and kinetic energy trade as height changes. Mechanical energy decreases when loss per meter is nonzero.");plt.close(energy)
    speed=series_figure(x=distance,series={"Speed":list(r.speed_m_s)},x_label="Distance along track (m)",y_label="Speed (m/s)",title="Speed along the modeled track");render_chart(speed,"Speed rises on descents and falls while climbing; the series ends at the first unreachable sample.");plt.close(speed)
def model():
