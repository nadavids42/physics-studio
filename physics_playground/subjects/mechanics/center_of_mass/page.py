...
def animation(r,seed):
    p=r.parameters;objects=[{"label":"Mass 1","mass":p.mass_1_kg,"position":p.position_1_m,"role":"accent","selected":p.mass_1_kg>=p.mass_2_kg},{"label":"Mass 2","mass":p.mass_2_kg,"position":p.position_2_m,"role":"energy","selected":p.mass_2_kg>p.mass_1_kg}]
    if p.mass_3_kg>0:objects.append({"label":"Mass 3","mass":p.mass_3_kg,"position":p.position_3_m,"role":"displacement","selected":False})
    show(document("center of mass",[],message=r.outcome,seed=seed,autoplay=False,scene_config={"objects":objects,"centerM":r.center_of_mass_m}))
def explore():
...
def analyze():
    mode_heading(LearningMode.ANALYZE,"Move one mass");positions=[x/2 for x in range(-10,11)];centers=[simulate(CenterOfMassParameters(2,-2,3,x)).center_of_mass_m for x in positions];figure=series_figure(x=positions,series={"Center of mass":centers},x_label="Mass 2 position (m)",y_label="Center position (m)",title="Center position as mass 2 moves");render_chart(figure,"Moving the second mass right shifts the center of mass right in a straight-line relationship.");plt.close(figure)
def model():
