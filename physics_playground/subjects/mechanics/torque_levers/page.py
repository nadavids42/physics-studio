...
def explore():
    ...
    with c2:effort=st.slider("Effort force (N)",0.,500.,80.,10.);ea=st.slider("Effort arm (m)",.1,3.,1.5,.1)
    r=simulate(LeverParameters(load,la,effort,ea));metric_table({"Load torque":f"{r.load_torque_n_m:.1f} N·m","Effort torque":f"{r.effort_torque_n_m:.1f} N·m","Advantage":f"{r.mechanical_advantage:.2f}×"});st.caption("Text outcome: "+r.outcome);animation(r,20262101);obs=st.text_input("Optional notebook observation",key="lever_obs")
    if st.button("▶ Test lever",type="primary",use_container_width=True):record(ID,r.parameters.to_dict(),st.session_state.get("lever_quiz_guess"),r.outcome,values(r),kidtools.process_run(ID,evaluate(r)),20262101,VERSION,obs);st.rerun()
    kidtools.mission_checklist("Torque and Levers")
def compare():
    mode_heading(LearningMode.COMPARE,"Short versus long effort arm");a=simulate(LeverParameters(effort_arm_m=.5));b=simulate(LeverParameters(effort_arm_m=2.5));changed_variable_banner(ChangedVariable("Effort arm","0.5 m","2.5 m"));metric_table({"Run A torque":f"{a.effort_torque_n_m:.1f} N·m","Run B torque":f"{b.effort_torque_n_m:.1f} N·m"})
...
def render():
    st.header("⚖️ Torque and Levers");revealed=kidtools.prediction_quiz(key="lever_quiz",question="With the same force, where should you push to create more turning effect?",options=["Near the pivot","Far from the pivot","Distance does not matter"],correct_index=1,reveal_text="Torque equals force times perpendicular distance from the pivot.",mission_id="lever_predict")
    if not revealed:return
    mode=mode_navigation(key="lever_mode");{LearningMode.EXPLORE:explore,LearningMode.COMPARE:compare,LearningMode.ANALYZE:analyze,LearningMode.MODEL:model}[mode]()
