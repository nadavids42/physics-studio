...
const scene={draw(ctx,s){
  ...
  if(kind==='ramp'){
    ...
    const car=s.tracks.car,pad=62,top=48,bottom=h-58,length=c.trackLengthM||1,maxHeight=Math.max(1,...(c.trackPoints||[]).map(q=>q.height)),X=d=>pad+d/length*(w-pad*2),Y=v=>bottom-v/maxHeight*(bottom-top),points=(c.trackPoints||[]).map(q=>({x:X(q.distance),y:Y(q.height)})),reachable=Math.max(0,Math.min(length,c.reachableDistanceM??length));
    const heightAt=d=>{for(let i=1;i<(c.trackPoints||[]).length;i++){const a=c.trackPoints[i-1],b=c.trackPoints[i];if(d<=b.distance){const u=(d-a.distance)/(b.distance-a.distance);return a.height+(b.height-a.height)*u}}return c.trackPoints?.at(-1)?.height||0};
    const reachablePoints=points.filter((_,i)=>(c.trackPoints[i].distance<=reachable));if(reachable<length){const hStop=heightAt(reachable),stop={x:X(reachable),y:Y(hStop)};if(!reachablePoints.length||reachablePoints.at(-1).x!==stop.x)reachablePoints.push(stop);const unavailable=[stop,...points.filter((_,i)=>c.trackPoints[i].distance>reachable)];PhysicsAssets.track(ctx,s,{points:unavailable,dashed:true,opacity:.42});PhysicsAssets.callout(ctx,s,{x:Math.min(w-205,stop.x+12),y:Math.max(22,stop.y-82),width:184,height:52,text:'Unreachable with\ncurrent energy',target:stop});}
    PhysicsAssets.track(ctx,s,{points:reachablePoints});PhysicsAssets.ruler(ctx,s,{x:pad-20,y:bottom,width:bottom-top,rotation:-Math.PI/2,divisions:5,maximum:maxHeight,showValues:PhysicsVisuals.responsive(s)!=='mobile'});
    const distance=(car?.x||0)*length,height=heightAt(distance),speed=car?.y||0,x=X(distance),y=Y(height)-18,next=Math.min(length,distance+length*.01),slope=Math.atan2(Y(heightAt(next))-Y(height),X(next)-X(distance)),pe=(c.massKg||0)*(c.gravityMps2||9.81)*height,ke=.5*(c.massKg||0)*speed*speed,lost=(c.lossPerMeterJ||0)*distance;PhysicsAssets.cart(ctx,s,{x,y,rotation:slope,width:56,height:28,label:'coaster'});
    if(speed>0)PhysicsAnnotations.motionDirection(ctx,s,{x:x+18*Math.cos(slope),y:y+18*Math.sin(slope),end:{x:x+54*Math.cos(slope),y:y+54*Math.sin(slope)},label:`${speed.toFixed(1)} m/s`});
    PhysicsAssets.callout(ctx,s,{x:18,y:20,width:210,height:68,text:`Height ${height.toFixed(1)} m\nPE ${pe.toFixed(0)} J · KE ${ke.toFixed(0)} J\nLost ${lost.toFixed(0)} J`});
    if((c.lossPerMeterJ||0)>0)PhysicsAnnotations.disclosure(ctx,s,'schematic','Mechanical energy decreases by the modeled loss per meter',12,h-34);
