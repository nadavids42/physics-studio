...
def dataclass_from_dict(model: type, payload: dict[str, Any]):
    ...
    hints=get_type_hints(model);known={item.name for item in fields(model)};unknown=set(payload)-known
    if unknown:raise ValueError(f"Unknown {model.__name__} fields: {', '.join(sorted(unknown))}")
    def restore(annotation,value):
        if value is None:return None
        origin=get_origin(annotation);arguments=get_args(annotation)
        ...
        if isinstance(annotation,type) and is_dataclass(annotation):return dataclass_from_dict(annotation,value)
        return value
