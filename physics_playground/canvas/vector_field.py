...
const reveal=s.reducedMotion?1:Math.max(.05,s.fraction);for(const p of c.samples){const mag=Math.hypot(p.ex,p.ey);if(!mag)continue;const relative=Math.min(1,Math.log10(1+mag)/Math.log10(1+c.fieldMagnitudeLimit)),len=(5+14*relative)*reveal,dx=p.ex/mag*len,dy=-p.ey/mag*len,x=X(p.x),y=Y(p.y);PhysicsVisuals.arrow(ctx,s,{x:x-dx*.3,y:y-dy*.3},{x:x+dx*.7,y:y+dy*.7},{role:'electric_field',width:1.35,head:5})}
...
"""
def build_vector_field_document(*,samples:list[dict],charges:list[dict],grid_size:int,extent:float,test_x:float,test_y:float,message:str,seed:int,test_charge:float=1.)->str:
    finite=[item for item in samples if item.get("v") is not None and item.get("ex") is not None and item.get("ey") is not None]
    excluded=[{"x":item["x"],"y":item["y"]} for item in samples if item.get("v") is None]
    potential_limit=max((abs(float(item["v"])) for item in finite),default=1.) or 1.
    field_limit=max((math.hypot(float(item["ex"]),float(item["ey"])) for item in finite),default=1.) or 1.
    config={"durationMs":1600,"autoplay":False,"seed":seed,"tracks":[{"id":"field-reveal","label":"Electric field reveal","x":[0,1]}],"events":[],"completionMessage":message,"vectorField":{"samples":finite,"excluded":excluded,"charges":charges,"gridSize":grid_size,"extent":extent,"testX":test_x,"testY":test_y,"testCharge":test_charge,"potentialLimit":potential_limit,"fieldMagnitudeLimit":field_limit},"view":{"minimum":0,"maximum":1}}
