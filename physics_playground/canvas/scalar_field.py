...
def build_scalar_field_document(*, x: tuple[float, ...], frames: tuple[tuple[float, ...], ...],
        duration_s: float, accessible_label: str, completion_message: str, seed: int,
        autoplay: bool = False, source_frames: tuple[tuple[tuple[float, ...], ...], ...] = (),
        sources: tuple[dict, ...] = (), measurements: tuple[dict, ...] = (),
        time_s: tuple[float, ...] = (), interference_label: str = "") -> str:
    ...
    recorded_limit = max((abs(value) for frame in frames for value in frame), default=1.)
    ...
    config = {"durationMs": max(1000, int(duration_s*2500)), "fieldDurationS": duration_s,
              "autoplay": autoplay, "seed": seed, "frameCount": len(frames),
              "tracks": [{"id": "field-clock", "label": "Wave field", "x": [0, 1]}],
              "events": [], "completionMessage": completion_message, "fieldX": x,
              "fieldFrames": frames, "sourceFrames": source_frames, "sources": sources,
              "measurements": measurements, "fieldTimes": times,
              "interferenceLabel": interference_label, "fieldLimit": limit,
              "view": {"minimum": 0, "maximum": 1}}
    return build_player_document(config=config, scene_javascript=SCENE, logical_width=820,
        logical_height=390, accessible_label=accessible_label,
