...
const limit=PhysicsExperience.level(s)==='diagram'?Math.min(12,c.particleLayout.length):c.particleLayout.length;for(let i=0;i<limit;i++){const particle=c.particleLayout[i],x=left+12+particle.u*(right-left-24),y=top+12+particle.v*Math.max(8,bottom-top-24);ctx.fillStyle=PhysicsVisuals.token(s,'colors','graph_2','#D55E00');ctx.globalAlpha=.78;ctx.beginPath();ctx.arc(x,y,particle.r,0,Math.PI*2);ctx.fill()}ctx.globalAlpha=1;
const arrowRatio=pressure/Math.max(c.pressureA,c.pressureB,1),arrowLength=24+28*arrowRatio;for(const x of [left+(right-left)*.25,left+(right-left)*.5,left+(right-left)*.75])PhysicsAnnotations.vector(ctx,s,{x,y:top-arrowLength-12,dx:0,dy:-1,role:'net_force',scale_mode:'normalized',fixed_length_px:arrowLength,scale_disclosure:'Pressure indicators are normalized for visibility'},1,false);
ctx.save();PhysicsVisuals.applyText(ctx,s,'label');const compact=PhysicsVisuals.responsive(s)==='mobile';ctx.fillText(`P ${pressure.toFixed(1)} kPa`,compact?16:left,32);ctx.fillText(`V ${(v*1000).toFixed(1)} L`,compact?16:left+150,compact?52:32);ctx.fillText(`T ${temp.toFixed(1)} K`,compact?16:left+285,compact?72:32);ctx.fillText(`n ${c.amount.toFixed(2)} mol`,compact?16:left+430,compact?92:32);ctx.restore();
