...
def _document(results: list[tuple[str, SpringResult, str]], *, seed: int, autoplay: bool, message: str) -> str:
    ...
    for i, (label, result, _color) in enumerate(results):
        ...
        tracks.append(track)
        p = result.parameters
        oscillators.append({"initialDisplacementM": p.initial_displacement_m,
    ...
    config = {"durationMs": 4200, "autoplay": autoplay, "seed": seed, "trailLength": 12,
              "view": {"minimum": minimum, "maximum": maximum}, "tracks": tracks,
              "boing": {"oscillators": oscillators}, "events": [], "completionMessage": message}
...
def build_boing_canvas(result: SpringResult, *, seed: int, autoplay: bool) -> str:
    return _document([("Spring mass", result, "")], seed=seed, autoplay=autoplay,
                     message="Oscillation complete!")


def build_boing_comparison_canvas(a: SpringResult, b: SpringResult, *, labels: tuple[str, str],
                                  seed: int, autoplay: bool) -> str:
    return _document([(labels[0], a, ""), (labels[1], b, "")], seed=seed, autoplay=autoplay,
