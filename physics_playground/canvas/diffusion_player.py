...
def build_diffusion_document(*, paths, dimensions: int, extent: float, message: str, seed: int,
                             rms_distance_m: float = 0,
                             sampled_particle_count: int | None = None) -> str:
    ...
    config = {"durationMs": 2400, "autoplay": False, "seed": seed,
              "frameCount": max((len(path) for path in immutable_paths), default=2),
              "tracks": [{"id": "walks", "label": "Particle trajectories", "x": [0, 1]}],
              "events": [], "completionMessage": message,
              "diffusion": {"paths": immutable_paths, "dimensions": dimensions, "extent": extent,
    ...
    label = ("1-dimensional random-walk trajectories constrained to the labeled horizontal axis. " if dimensions == 1
             else "2-dimensional random-walk trajectories on a labeled coordinate grid. ")
    return build_player_document(config=config, scene_javascript=SCENE, logical_width=760,
        logical_height=500,
        accessible_label=label+"Trajectories use numbered solid and dashed identities; the RMS guide describes the statistical distribution. "+message,
