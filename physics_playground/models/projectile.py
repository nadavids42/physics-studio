...
class ProjectileParameters:
    ...
    def validate(self) -> None:
        ...
        enforce_budget(
            "Projectile integration steps",
            math.ceil(self.max_time_s / self.time_step_s),
            MAX_INTEGRATION_STEPS,
        )
...
def _build_result(
    ...
    *,
    ...
    energy = (
        0.5 * parameters.mass_kg * speed**2 + parameters.mass_kg * parameters.gravity_m_s2 * y_m
    )
    range_m = float(x_m[-1])
    ...
    warnings = (
        () if landed else ("The projectile did not reach the ground before the simulation limit.",)
    )
    ...
    if landed:
        ...
        events.append(
            SimulationEvent(
                "not_landed", EventKind.WARNING, flight_time, "Simulation ended before impact"
            )
        )
    plots = (
        PlotData(
    ...
    return ProjectileResult(
        ...
        metrics=(
            ...
            SummaryMetric(
                ...
                max(0.0, float(energy[0] - energy[-1])),
                "J",
            ),
        ),
        events=tuple(events),
        ...
        animation=AnimationData(
            ...
            {
                "x_min": 0.0,
                "x_max": max(1.0, range_m * 1.12),
                "y_min": 0.0,
                "y_max": max(1.0, maximum_height * 1.25),
            },
        ),
        ...
        outcome_description=(
            ...
            if landed
            else "The projectile did not land before the simulation limit."
        ),
...
def simulate_no_drag(parameters: ProjectileParameters) -> ProjectileResult:
    ...
    return _build_result(
        ...
        x_m,
        y_m,
        np.full_like(time_s, vx),
        vy - parameters.gravity_m_s2 * time_s,
        landed=True,
        method="No drag (analytic)",
    )
...
def _derivative(state: np.ndarray, parameters: ProjectileParameters) -> np.ndarray:
    ...
    return np.array(
        (vx, vy, -factor * speed * vx, -parameters.gravity_m_s2 - factor * speed * vy), dtype=float
    )
...
def simulate_quadratic_drag(parameters: ProjectileParameters) -> ProjectileResult:
    ...
    state = np.array(
        (
            0.0,
            parameters.initial_height_m,
            parameters.launch_speed_m_s * math.cos(theta),
            parameters.launch_speed_m_s * math.sin(theta),
        ),
        dtype=float,
    )
    states = [state.copy()]
    ...
    return _build_result(
        ...
        array[:, 3],
        landed=landed,
        method="Quadratic drag (RK4)",
    )


def simulate_projectile(parameters: ProjectileParameters) -> ProjectileResult:
    return (
        simulate_no_drag(parameters)
        if parameters.drag_coefficient_kg_m == 0
        else simulate_quadratic_drag(parameters)
    )
