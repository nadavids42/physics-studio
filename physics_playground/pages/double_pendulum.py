...
def _summary(r):
    c = st.columns(4)
    c[0].metric("Energy drift", f"{r.energy_drift_fraction * 100:.4f}%")
    c[1].metric("Angular separation", f"{r.final_angular_separation_rad:.3f} rad")
    c[2].metric("Cartesian separation", f"{r.final_cartesian_separation_m:.3f} m")
    c[3].metric(
        "Divergence rate",
        f"{r.divergence_rate_per_s:.3f} 1/s" if r.divergence_rate_per_s is not None else "n/a",
    )
    st.caption(
        f"Text outcome: the paired pendulums finish {r.final_cartesian_separation_m:.3f} meters apart with angular separation {r.final_angular_separation_rad:.3f} radians."
    )
...
def _metrics(r):
    return {
        ...
        "angular_separation_rad": r.final_angular_separation_rad,
        "cartesian_separation_m": r.final_cartesian_separation_m,
        "divergence_rate_per_s": r.divergence_rate_per_s or 0,
    }
...
def _reuse():
    ...
    if not q or q.get("simulation_id") != "double_pendulum":
        return
    p = q["parameters"]
    for key, name in (
...
def _pair(kind):
    if kind == "Different perturbation sizes":
        a = DoublePendulumParameters(perturbation_deg=0.01)
        b = DoublePendulumParameters(perturbation_deg=1)
        change = ChangedVariable("Perturbation", "0.01°", "1°")
    elif kind == "Different initial angles":
        a = DoublePendulumParameters(angle_1_deg=90)
        b = DoublePendulumParameters(angle_1_deg=130)
        change = ChangedVariable("Initial angle 1", "90°", "130°")
    elif kind == "Different mass ratios":
        a = DoublePendulumParameters(mass_1_kg=1, mass_2_kg=0.3)
        b = DoublePendulumParameters(mass_1_kg=1, mass_2_kg=3)
        change = ChangedVariable("Mass ratio m₂/m₁", "0.3", "3")
    else:
...
def render_compare():
    mode_heading(LearningMode.COMPARE, "Compare chaos experiments")
    kind = st.selectbox(
        "Comparison",
        [
...
def render_model():
    mode_heading(LearningMode.MODEL, "Coupled nonlinear equations and sensitivity")
    st.markdown(
        "The two coupled second-order equations are integrated as four first-order equations with RK4. Tiny initial differences grow because the nonlinear flow stretches nearby trajectories through phase space."
    )
    st.latex(
    ...
    assumptions_panel(
        assumptions,
        (
            "Rods cannot bend or collide.",
            "Air resistance and pivot friction are omitted.",
            "The fitted divergence rate is finite-time and only approximate.",
            "Chaos makes long-term trajectories timestep-sensitive even when conserved quantities look good.",
        ),
    )
...
def render():
    ...
    revealed = kidtools.prediction_quiz(
        ...
        options=[
            ...
            "They can separate dramatically",
            "They become perfectly synchronized",
        ],
        correct_index=1,
        reveal_text="Nonlinear dynamics can amplify a microscopic difference into completely different motion.",
        mission_id="chaos_predict",
    )
    if not revealed:
