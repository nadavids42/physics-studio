...
def plot_figure(plot,overlays=())->Figure:
    ...
    for item in (plot,*overlays):
        for s in item.series:ax.plot(s.x,s.y,label=s.label)
...
def error_figure(length,gravity)->Figure:
    a,e=approximation_error_curve(length,gravity);fig,ax=plt.subplots();ax.plot(a,e);ax.set_xlabel("Release angle (degrees)");ax.set_ylabel("Period error (%)");ax.set_title("Small-angle approximation error");ax.grid(True);return fig
