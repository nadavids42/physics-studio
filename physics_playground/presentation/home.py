...
def _filtered():
    concepts = sorted({concept for item in SIMULATION_REGISTRY for concept in item.concepts})
    types = sorted({item.simulation_type for item in SIMULATION_REGISTRY})
    ...
    with c3:
        status = st.selectbox("Progress", ["All missions", "Completed", "Incomplete"])
    with c4:
        kind = st.selectbox("Simulation type", ["All types", *types])
    result = []
    for item in SIMULATION_REGISTRY:
        _, _, fraction = _progress(item)
        if concept != "All concepts" and concept not in item.concepts:
            continue
        if difficulty != "All levels" and item.difficulty.value != difficulty:
            continue
        if status == "Completed" and fraction < 1:
            continue
        if status == "Incomplete" and fraction >= 1:
            continue
        if kind != "All types" and item.simulation_type != kind:
            continue
        result.append(item)
    return result
...
def _card(item: SimulationDefinition) -> None:
    ...
    with st.container(border=True):
        ...
        if visual:
            st.caption(f"{visual.symbol} {visual.thumbnail_alt}")
