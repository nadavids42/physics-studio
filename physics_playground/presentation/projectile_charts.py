...
def plot_figure(plot: PlotData, overlays: tuple[PlotData, ...] = ()) -> Figure:
    ...
    if overlays or len(plot.series)>1: ax.legend()
    return fig
...
def range_by_angle_figure(speed_m_s: float, gravity_m_s2: float) -> Figure:
    ...
    fig,ax=plt.subplots(); ax.plot(list(angles),ranges); ax.axvline(45,color="crimson",linestyle="--");
    ax.set_xlabel("Launch angle (degrees)"); ax.set_ylabel("Range (m)"); ax.set_title("Maximum no-drag range occurs near 45°"); ax.grid(True); return fig
