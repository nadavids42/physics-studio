...
def validate_expansion_definition(definition: ExpansionDefinition) -> None:
    ...
    if declared_modes != required_modes or not required_modes.issubset(metadata.modes):
        raise PhysicsValidationError("Every expansion simulation must implement exactly the four learning modes.")
    ...
    if not definition.parameter_model or not definition.result_model or not definition.physics_entrypoint:
    ...
    if definition.tests.analytic_or_invariant_tests < 2 or definition.tests.validation_tests < 3:
        raise PhysicsValidationError("Expansion simulations need physics invariants and boundary validation tests.")
    if PresentationCapability.PREDICTION not in next(
        ...
        raise PhysicsValidationError("Explore mode must collect a prediction before revealing results.")
