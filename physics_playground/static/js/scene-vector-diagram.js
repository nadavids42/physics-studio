(() => {
  ...
  // src/scenes/shared/vector-diagram.js
  var scene = {
    draw(ctx, s) {
      const w = s.transform.width, h = s.transform.height, c = s.config.vectorDiagram || {}, cx = w / 2, cy = h / 2 - 15;
      ...
      ctx.fillStyle = PhysicsVisuals.token(
        s,
        "colors",
        c.forceZ > 0 ? "normal_force" : c.forceZ < 0 ? "magnetic_field" : "text_muted",
        "#526577"
      );
      ctx.font = "650 58px " + PhysicsVisuals.token(s, "typography", "family_ui", "system-ui");
      ...
      ctx.restore();
      if (PhysicsExperience.level(s) !== "diagram" && PhysicsVisuals.responsive(s) !== "mobile")
        PhysicsAssets.callout(ctx, s, {
          x: w - 260,
          y: 18,
          width: 245,
          height: 62,
          text: "Right-hand rule\n" + (c.subjectKind === "charge" ? "Follows v \xD7 B; reverse for q < 0" : "Follows I \xD7 B; reverse for I < 0"),
          shadow: true
        });
    }
  };
