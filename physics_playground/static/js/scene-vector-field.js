(() => {
  ...
  // src/scenes/shared/vector-field.js
  var scene = {
    draw(ctx, s) {
      const w = s.transform.width, h = s.transform.height, c = s.config.vectorField || {}, n = c.gridSize || 1, pad = 38, cellW = (w - 2 * pad) / Math.max(1, n - 1), cellH = (h - 2 * pad) / Math.max(1, n - 1), X = (x) => pad + (x + c.extent) / (2 * c.extent) * (w - 2 * pad), Y = (y) => h - pad - (y + c.extent) / (2 * c.extent) * (h - 2 * pad);
      ...
      for (const p of c.samples) {
        const ratio = Math.min(1, Math.abs(p.v) / c.potentialLimit), color = PhysicsVisuals.token(
          s,
          "colors",
      ...
      for (const p of c.samples) {
        ...
        ), len = (5 + 14 * relative) * reveal, x = X(p.x), y = Y(p.y);
        PhysicsAnnotations.vector(
          ...
          s,
          {
            ...
            fixed_length_px: len,
            scale_disclosure: "Field directions are physical; arrow lengths are normalized for visibility using logarithmic magnitude"
          },
          1,
          false
        );
      ...
      for (const q of c.charges)
        PhysicsAssets.charge(ctx, s, {
          ...
          sign: q.q,
          shadow: true,
          label: PhysicsVisuals.responsive(s) === "mobile" ? "" : q.q > 0 ? "+ source" : "\u2212 source"
        });
      PhysicsAssets.charge(ctx, s, {
        x: X(c.testX),
        y: Y(c.testY),
