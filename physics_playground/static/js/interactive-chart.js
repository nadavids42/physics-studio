(() => {
  // src/interactive-chart.js
  var SVG_NS = "http://www.w3.org/2000/svg";
  var COLORS = ["#0072B2", "#D55E00", "#00875A", "#A23B72", "#8A5A00"];
  var SEMANTIC_COLORS = {
    graph_1: "#0072B2",
    graph_2: "#D55E00",
    graph_3: "#00875A",
  ...
  var DASHES = { solid: "", dashed: "9 6", dotted: "2 5" };
  ...
  function zoomedDomain(current, center, factor) {
  ...
  function mountInteractiveChart(config) {
    document.body.classList.toggle(
      "high-contrast",
      Boolean(config.high_contrast)
    );
    const svg = document.getElementById("chart"), tableHost = document.getElementById("table"), tooltip = document.getElementById("tooltip"), reset = document.getElementById("zoom-reset");
    if (!svg || !config?.series?.length)
      throw new TypeError("Interactive chart payload is invalid.");
    const margin = { left: 72, right: 24, top: 34, bottom: 58 }, width = 720, height = 390, allX = config.series.flatMap((item) => item.x), allY = config.series.flatMap((item) => item.y), originalX = domain(allX), yDomain = domain(allY);
    let xDomain = [...originalX];
    const dark = config.theme === "dark" || config.theme === "auto" && globalThis.matchMedia?.("(prefers-color-scheme: dark)").matches;
    const draw = () => {
      svg.querySelectorAll("g.chart-data").forEach((node) => node.remove());
      const group = svgElement("g", { class: "chart-data" }), X = (x) => margin.left + (x - xDomain[0]) / (xDomain[1] - xDomain[0]) * (width - margin.left - margin.right), Y = (y) => height - margin.bottom - (y - yDomain[0]) / (yDomain[1] - yDomain[0]) * (height - margin.top - margin.bottom);
      for (let tick = 0; tick <= 5; tick++) {
        const x = margin.left + tick / 5 * (width - margin.left - margin.right), y = margin.top + tick / 5 * (height - margin.top - margin.bottom);
        group.append(
          svgElement("line", {
            x1: x,
            x2: x,
        ...
        xTick.textContent = (xDomain[0] + tick / 5 * (xDomain[1] - xDomain[0])).toPrecision(3);
        const yTick = svgElement("text", {
          x: margin.left - 10,
          y: y + 4,
          "text-anchor": "end"
        });
        yTick.textContent = (yDomain[1] - tick / 5 * (yDomain[1] - yDomain[0])).toPrecision(3);
      ...
      config.series.forEach((item, seriesIndex) => {
        const palette = dark ? DARK_COLORS : SEMANTIC_COLORS, color = config.high_contrast ? HIGH_CONTRAST_COLORS[seriesIndex % HIGH_CONTRAST_COLORS.length] : palette[item.semantic_color] || COLORS[seriesIndex % COLORS.length], visible = item.x.map((x, i) => ({ x, y: item.y[i] })).filter((point) => point.x >= xDomain[0] && point.x <= xDomain[1]);
      ...
      svg.append(group);
      svg.onpointermove = config.hover ? (event) => {
        const rect = svg.getBoundingClientRect(), px = (event.clientX - rect.left) / rect.width * width, value = xDomain[0] + (px - margin.left) / (width - margin.left - margin.right) * (xDomain[1] - xDomain[0]), nearest = nearestPoint(config.series, value);
        if (!nearest) return;
      ...
      if (config.zoom)
        svg.onwheel = (event) => {
          ...
          const rect = svg.getBoundingClientRect(), center = xDomain[0] + (event.clientX - rect.left) / rect.width * (xDomain[1] - xDomain[0]);
    ...
    const highlight = (id) => {
      ...
      svg.querySelectorAll(".series,.point").forEach(
        (node) => node.classList.toggle(
          "muted",
          Boolean(id) && !node.classList.contains(`series-${id}`)
        )
      );
      tableHost.querySelectorAll("tr[data-series]").forEach(
        (row) => row.classList.toggle("muted", Boolean(id) && row.dataset.series !== id)
      );
    };
    if (config.table) {
      const rows = config.series.flatMap(
        (item) => item.x.map(
          (x, i) => `<tr tabindex="0" data-series="${item.id}"><td>${item.label}</td><td>${x.toFixed(4)}</td><td>${item.y[i].toFixed(4)}</td></tr>`
        )
      ).join("");
