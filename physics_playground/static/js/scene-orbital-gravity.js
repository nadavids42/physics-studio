(() => {
  ...
  // src/scenes/mechanics/orbital-gravity.js
  var scene = {
    draw(ctx, f) {
      const t = f.transform, W = t.width, H = t.height, cx = W / 2, cy = H / 2, view = f.config.orbitView, scale = Math.min(W, H) * 0.43 / view;
      ...
      for (const q of Object.values(f.tracks)) {
        const color = q.style.color || PhysicsVisuals.token(f, "colors", "trajectory", "#1769AA");
        ...
        PhysicsAssets.planet(ctx, f, {
          ...
          highlight: true,
          shadow: true,
          label: f.config.tracks.length > 1 && PhysicsVisuals.responsive(f) !== "mobile" ? q.label : ""
        });
      }
    }
  };
