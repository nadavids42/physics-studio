(() => {
  ...
  // src/scenes/shared/fluid-container.js
  var scene = {
    draw(ctx, s) {
      const w = s.transform.width, h = s.transform.height, c = s.config.fluidContainer || {}, left = w * 0.22, right = w * 0.78, top = h * 0.15, bottom = h * 0.82, width = right - left, height = bottom - top, waterline = top + 42;
      PhysicsExperience.context(ctx, s, "laboratory");
      if (c.kind === "buoyancy") {
        const progress = s.reducedMotion ? 1 : s.fraction, fraction = Math.max(0, Math.min(1, c.fraction)), objectH = 96, objectW = Math.min(116, width * 0.42);
        let cy;
        ...
        const submergedTop = Math.max(waterline, cy - objectH / 2), submergedBottom = Math.min(bottom, cy + objectH / 2);
        ...
        const maxForce = Math.max(c.weight, c.buoyant, 1), weightLength = 30 + 40 * c.weight / maxForce, buoyantLength = 30 + 40 * c.buoyant / maxForce, vectors = [
          {
            x: w / 2,
            y: cy,
            dx: 0,
        ...
        if (PhysicsVisuals.responsive(s) !== "mobile")
          PhysicsAssets.callout(ctx, s, {
            ...
            width: 190,
            height: 82,
            text: `Object: ${c.objectDensity.toFixed(0)} kg/m\xB3
Fluid: ${c.fluidDensity.toFixed(0)} kg/m\xB3
Displaced: ${c.displacedVolume.toFixed(4)} m\xB3`,
            shadow: false
          });
      } else {
        ...
        for (const sample2 of samples) {
          const y = top + height * (sample2.depth / c.maxDepth), length = 18 + 54 * sample2.ratio;
          PhysicsAnnotations.vector(
            ...
            s,
            {
              ...
              scale_disclosure: "Pressure-arrow lengths are normalized for visibility"
            },
            1,
            false
          );
        ...
        ctx.fillText(
          `Surface: ${(c.surfacePressure / 1e3).toFixed(2)} kPa`,
          left + 12,
          top + 24
        );
        ctx.fillText(
          `Gauge: ${(c.gaugePressure / 1e3).toFixed(2)} kPa`,
          left + 12,
          selectedY - 24
        );
        ctx.fillText(
          `Absolute: ${(c.absolutePressure / 1e3).toFixed(2)} kPa`,
          left + 12,
          selectedY - 8
        );
        ctx.restore();
