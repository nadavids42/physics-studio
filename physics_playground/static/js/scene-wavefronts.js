(() => {
  ...
  // src/scenes/shared/wavefronts.js
  var scene = {
    draw(ctx, s) {
      const w = s.transform.width, h = s.transform.height, c = s.config, frames = c.wavefrontFrames || [], pad = 52, axisY = h * 0.58, min = c.worldMin, max = c.worldMax, map = (x) => pad + (x - min) / (max - min) * (w - pad * 2), metersToPixels = (r) => Math.abs(map(r) - map(0));
      ...
      const frame = frames[Math.min(
        frames.length - 1,
        Math.round(s.fraction * (frames.length - 1))
      )];
      for (let i = 0; i < frame.centers.length; i++) {
        const center = map(frame.centers[i]), radius = metersToPixels(frame.radii[i]);
      ...
      const sx = map(frame.source), ox = map(frame.observer);
      ...
      if (Math.abs(c.sourceVelocityMps) > 1e-9)
        PhysicsAnnotations.vector(
          ...
          s,
          {
            ...
            fixed_length_px: 48,
            scale_disclosure: "Motion-arrow direction is physical; length is normalized"
          },
          s.progress,
          true
        );
      ...
      const ahead = c.sourceVelocityMps > 0 ? "compressed ahead" : c.sourceVelocityMps < 0 ? "expanded ahead" : "equal spacing ahead", behind = c.sourceVelocityMps > 0 ? "expanded behind" : c.sourceVelocityMps < 0 ? "compressed behind" : "equal spacing behind";
...
\u03BB = ${c.wavelengthAheadM.toFixed(2)} m`
      ...
      ctx.fillStyle = PhysicsVisuals.token(s, "colors", "text", "#152536");
      ctx.fillText(`${c.motionLabel} \xB7 positions and radii physical`, pad, 25);
      ctx.restore();
