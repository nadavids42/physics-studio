(() => {
  ...
  // src/scenes/shared/ray-diagram.js
  var scene = {
    draw(ctx, s) {
      const w = s.transform.width, h = s.transform.height, c = s.config.rayConfig || {}, rays = c.rays || [], xmin = c.xmin ?? -5, xmax = c.xmax ?? 5, ymin = c.ymin ?? -4, ymax = c.ymax ?? 4, X = (x) => 35 + (x - xmin) / (xmax - xmin) * (w - 70), Y = (y) => h - 30 - (y - ymin) / (ymax - ymin) * (h - 60), origin = { x: X(0), y: Y(0) };
      ...
      if (c.interface) {
        ...
        PhysicsAnnotations.angleArc(ctx, s, {
          ...
          endAngle: -Math.PI / 2,
          label: `${c.incidentAngle.toFixed(0)}\xB0`
        });
        PhysicsAnnotations.angleArc(ctx, s, {
          x: origin.x,
          y: origin.y,
        ...
        if (c.refractionAngle !== null)
          PhysicsAnnotations.angleArc(ctx, s, {
            ...
            endAngle: Math.PI / 2,
            label: `${c.refractionAngle.toFixed(0)}\xB0`
          });
      } else {
      ...
      const visible = s.reducedMotion ? rays.length : Math.ceil(s.fraction * rays.length);
      for (let i = 0; i < visible; i++) {
        const r = rays[i], virtual = r.kind === "virtual", color = PhysicsVisuals.token(
          s,
          "colors",
